...
class AsistenteController extends Controller
{
    ...
    public function edit2(Request $request, $id)
    {
        ...
        if (isset($request->tipo)) {
            return redirect()->route('admin.Charlas.show',$request->idCharla);
        } else {
    ...
    public function look(Request $request)
    {
        ...
        if (isset($request->idCharla)) {
            $id = $request->idCharla;

            if (!$request->dni && !$request->nombre  && !$request->apePa) {
                session()->flash('swal', [
                    'icon' => 'error',
                    'title' => '¡Ups!',
                    'text' => 'No ingrese ningun dato'
                ]);
                
                return redirect()->route('admin.Charlas.show',$id);
            } else {
                $resultado = DB::select('EXEC dbo.LookAsistentesCharlas ?,?,?,?',[
                    $request->dni,
                    $request->nombre,
                    $request->apePa,
                    $id
                ...
                if (!empty($resultado)) {
                    ...
                    session()->flash('swal', [
                        'icon' => 'success',
                        'title' => 'Genial!',
                        'text' => 'Se encontro el asistente'
                    ]);
    
                    return view('admin/charla/AsistenteCharla', compact('cantidad','asitentes','charla','expositores','documentos','imagenes'));
